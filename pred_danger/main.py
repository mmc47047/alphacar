...
# PyTorch load 호환 함수 (PyTorch<2.0 에서 weights_only 미지원 대비)
...
def run_process_for_camera(df: pd.DataFrame, camera_type: str, config: dict):
    ...
    if len(X) < config['N_SPLITS'] or (len(y) > 0 and y.sum() < config['N_SPLITS']):
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
            ...
            cv_model.load_state_dict(torch_load_compat(temp_path, weights_only=True))
    ...
    final_model_path = f"./model/best_model_{camera_type}_classification.pth"
    final_scaler_path = f"./model/scaler_{camera_type}_classification.pkl"
    with open(final_scaler_path, 'wb') as f: 
        pickle.dump(scaler, f, protocol=4)  # protocol=4 추가 (Python 3.4+ 호환)
    ...
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    final_model.load_state_dict(torch_load_compat(final_model_path, weights_only=True))
