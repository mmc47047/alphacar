...
class SequenceGenerator:
    def __init__(self, seq_len=60, pred_horizon=60, stride=10, cam_type='both'):
    ...
    def create_sequences(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for vid in tqdm(df['vehicle_id'].unique(), desc=f"Analyzing trajectories for '{self.cam_type}'"):
            ...
            for i in range(0, len(features) - self.seq_len - self.pred_horizon, self.stride):
                end_idx = i + self.seq_len
                future_idx = end_idx + self.pred_horizon -1
                
                # 미래에 사고가 없는 경우에만 Negative 샘플로 추가
                if future_idx < len(labels) and not np.any(labels[end_idx:future_idx+1]):
...
class CollisionTrainer: # ✨ 수정됨: 분류용으로 되돌림 (FocalLoss, Acc 추가)
    def __init__(self, model, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.model, self.device = model.to(device), device
        self.criterion = FocalLoss() # ✨ 손실 함수를 FocalLoss로
